...
console.log(route?.params?.monitor,'route ')
const isMonito=route?.params?.monitor||false
  const handleAddEditWorkout = (workout) => {
    setFitnessData((prevData) => {
      const existingIndex = prevData.findIndex((item) => item.id === workout.id);
      if (existingIndex >= 0) {
        const newData = [...prevData];
        newData[existingIndex] = workout;
        return newData;
      }
      return [...prevData, workout];
    });
  };

  console.log(fitnessData,'fitness data')
